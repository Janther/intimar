...
**Path bases differ**: the `path` attribute resolves relative to the _template file_, while a relative `--config.includePath` resolves relative to the _current working directory_. Denied includes are dropped silently (exit `0`, stderr warning only) — see `compilation.md`.
