...
The search script lives inside this skill's own directory, not the project directory. This is a project-vendored copy (not installed via the plugin system), so invoke it by the path below relative to the project root — not `${CLAUDE_PLUGIN_ROOT}`, which isn't set here:
